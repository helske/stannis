functions {
 ...
 real poisson_local_level_approx_lpmf(int[] y, real x1, real P1, real sd_x, vector theta_) {
   ...
   //mode term
   for(t in 1:n) {
     loglik = loglik + poisson_lpmf(y[t] | exp(theta[t])) - 
       normal_lpdf(approx_y[t] | theta[t], sqrt(approx_var_y[t]));
     //y[t] * theta[t] - exp(theta[t]) +
     //0.5 * (approx_y[t] - theta[t])^2 / approx_var_y[t] + 0.5 * log(approx_var_y[t]);
   }
   return loglik;
 }
 
 
  real poisson_local_level_approx(int[] y, real x1, real P1, real sd_x, vector theta_) {
...
model {
  ...
  target += poisson_local_level_approx_lpmf(y | x1, P1, sd_x, theta_init);
}
