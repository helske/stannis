...
stan_data <- list(n = n, y = y, x1 = 0, P1 = 10, prior_mean = 0, prior_sd = 10)
...
fit <- sampling(stannis:::stanmodels$poisson_ll, data = stan_data, seed = 1,
  refresh = 0, iter = 5000,  chains = 3, init = stan_inits)
...
stan_inits <- list(
  ...
  list(sd_x = 0.5),
  list(sd_x = 1))
fita <- sampling(stannis:::stanmodels$poisson_ll_approx, data = stan_data, seed = 1,
  refresh = 0, iter = 5000,  chains = 3, init = stan_inits)

c_time <- proc.time()
stan_out <- extract(fita, c("lp__", "sd_x", "jacobian", "approx_results"))
...
set.seed(1)
correction <- is_correction_psi(model, stan_out$approx_results, sd_x, stan_out$lp__, stan_out$jacobian, 10)
