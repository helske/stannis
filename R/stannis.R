...
stannis <- function(formula, data, level, slope, seasonal,
  distribution, x1, P1, beta, iter = 7500, thin = 5, nsim_states = 10, 
  stan_inits, n_threads = 1, is_seed = sample.int(.Machine$integer.max, 1), ...) {
  ...
  if (k==0) {
    ...
    correction <- is_correction(model, stan_out$approx_results, stan_out$theta, 
      stan_out$lp__, stan_out$jacobian, nsim_states, n_threads, is_seed)
    ...
    correction <- is_correction(model, stan_out$approx_results,
      cbind(stan_out$theta, stan_out$beta), stan_out$lp__, stan_out$jacobian, 
